...
export default function AccordionItem({ title, icon, children, isOpen, onToggle, id }: AccordionItemProps) {
  return (
    <motion.div
      layout
      className={cn(
        "bg-white/[0.02] border border-white/[0.06] rounded-[24px] overflow-hidden transition-all duration-300",
        isOpen ? "shadow-2xl border-blue-500/20 bg-white/[0.04]" : "hover:border-white/[0.1] hover:bg-white/[0.03]"
      )}
    >
      <button
        onClick={onToggle}

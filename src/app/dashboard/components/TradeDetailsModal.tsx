      ...
      <div className="space-y-8 py-4 overflow-y-auto max-h-[70vh] scrollbar-thin pr-2">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 bg-muted/30 rounded-xl border border-border">
          <div className="text-center">
            ...
            <p
              className={`text-lg font-bold ${Number(trade.pnl_amount) >= 0 ? 'text-green-400' : 'text-red-400'}`}
            >
              {Number(trade.pnl_amount) >= 0 ? '+' : ''}$
              {Math.abs(Number(trade.pnl_amount)).toFixed(2)}
            </p>
          </div>
          <div className="text-center">
            <p className="text-[10px] uppercase text-muted-foreground font-bold">RR Ratio</p>
            <p className="text-lg font-bold text-foreground">
              {Number(trade.rr_ratio).toFixed(2)}R
            </p>
          </div>
          <div className="text-center">
        ...
        {/* Screenshots */}
        {((trade.entry_images?.length ?? 0) > 0 ||
          (trade.exit_images?.length ?? 0) > 0 ||
          (trade.chart_images?.length ?? 0) > 0) && (
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {trade.entry_images.map((url, i) => (
                      <div
                        key={`entry-img-${i}`}
                        className="aspect-video relative rounded-lg overflow-hidden border border-border bg-muted"
                      >
                        <AppImage
                          src={url}
                          alt={`Entry screenshot ${i + 1}`}

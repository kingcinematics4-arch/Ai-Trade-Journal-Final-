...
function computeScores(trades: DbTrade[]): AiScores {
  ...
  const noMistakeCount = trades.filter(
  ...
  if (riskAmounts.length >= 2) {
    ...
    const variance =
      riskAmounts.reduce((a, r) => a + Math.pow(r - avgRisk, 2), 0) / riskAmounts.length;
  ...
  const avgWin =
    wins.length > 0 ? wins.reduce((a, t) => a + Math.abs(getTradePnL(t)), 0) / wins.length : 0;
  const avgLoss =
    losses.length > 0
      ? losses.reduce((a, t) => a + Math.abs(getTradePnL(t)), 0) / losses.length
      : 0;
  ...
  const calmTrades = emotionalTrades.filter((t) =>
    ['calm', 'confident', 'neutral'].includes((t.emotion_before ?? '').toLowerCase())
  );
  const revengeTrades = trades.filter(
    (t) =>
      (t.emotion_before ?? '').toLowerCase() === 'revenge' ||
      (t.mistake_category ?? '').toLowerCase().includes('revenge')
  );
  let emotionalControlScore =
    emotionalTrades.length > 0
      ? Math.round((calmTrades.length / emotionalTrades.length) * 100)
      : 50;
...
function generateFeedback(trades: DbTrade[]): AiCoachFeedback[] {
  ...
  if (parseFloat(winRate) >= 55) {
    feedback.push({
      message: `Your win rate of ${winRate}% is strong. Keep maintaining your edge.`,
      type: 'positive',
      category: 'Performance',
    });
  } else if (parseFloat(winRate) < 40 && totalDecided > 5) {
    feedback.push({
      message: `Your win rate is ${winRate}%. Focus on higher-probability setups and be more selective.`,
      type: 'warning',
      category: 'Performance',
    });
  }
  ...
  if (avgLoss > 0 && avgWin > 0) {
    ...
    if (avgLoss > avgWin * 1.5) {
      feedback.push({
        message: `Your average losing trade is ${ratio}x bigger than your average winner. Tighten your stop losses.`,
        type: 'negative',
        category: 'Risk Management',
      });
    } else if (avgWin > avgLoss * 1.5) {
      feedback.push({
        message: `Great risk-reward profile! Your winners are ${(avgWin / avgLoss).toFixed(1)}x larger than your losers.`,
        type: 'positive',
        category: 'Risk Management',
      });
    }
  ...
  if (bestStrategy) {
    ...
    feedback.push({
      message: `You perform best with ${bestStrategy[0]} strategy (${wr}% win rate across ${bestStrategy[1].total} trades).`,
      type: 'positive',
      category: 'Strategy',
    });
  }
  ...
  if (revengeTrades.length > 0) {
    feedback.push({
      message: `${revengeTrades.length} revenge trades detected. Step away after consecutive losses to reset mentally.`,
      type: 'negative',
      category: 'Psychology',
    });
  }
  ...
  if (overtradeDays.length > 0) {
    feedback.push({
      message: `Overtrading detected on ${overtradeDays.length} day(s). More trades don't mean more profit.`,
      type: 'warning',
      category: 'Discipline',
    });
  }
...
function analyzeDayPerformance(trades: DbTrade[]): DayPerformance[] {
  ...
  return days.map((day, idx) => ({
    ...
    winRate:
      dayData[idx].total > 0 ? Math.round((dayData[idx].wins / dayData[idx].total) * 100) : 0,
...
function analyzeStreaks(trades: DbTrade[]): StreakAnalysis {
  ...
  const lastStatus =
    sorted.length > 0 ? normalizeStatus(sorted[sorted.length - 1].trade_status) : 'none';
  let currentCount = 0;
  ...
  const afterWinWins = afterWinTrades.filter(
    (t) => normalizeStatus(t.trade_status) === 'win'
  ).length;
  const afterLossWins = afterLossTrades.filter(
    (t) => normalizeStatus(t.trade_status) === 'win'
  ).length;

  return {
    currentStreak: {
      type: lastStatus === 'win' || lastStatus === 'loss' ? lastStatus : 'none',
      count: currentCount,
    },
    maxWinStreak: maxWin,
    ...
    behaviorAfterWins: {
      winRate:
        afterWinTrades.length > 0 ? Math.round((afterWinWins / afterWinTrades.length) * 100) : 0,
      avgPnl:
        afterWinTrades.length > 0
          ? Math.round(
              (afterWinTrades.reduce((a, t) => a + getTradePnL(t), 0) / afterWinTrades.length) * 100
            ) / 100
          : 0,
    ...
    behaviorAfterLosses: {
      winRate:
        afterLossTrades.length > 0 ? Math.round((afterLossWins / afterLossTrades.length) * 100) : 0,
      avgPnl:
        afterLossTrades.length > 0
          ? Math.round(
              (afterLossTrades.reduce((a, t) => a + getTradePnL(t), 0) / afterLossTrades.length) *
                100
            ) / 100
          : 0,
...
function analyzeEmotions(trades: DbTrade[]): EmotionAnalysis {
  ...
  return {
    emotionDistribution: Array.from(emoMap.entries())
      .map(([emotion, data]) => ({
        emotion,
        count: data.count,
    ...
    calmTradeWinRate:
      calmData && calmData.count > 0 ? Math.round((calmData.wins / calmData.count) * 100) : 0,
  };
}
...
function generateSmartAlerts(trades: DbTrade[]): SmartAlert[] {
  ...
  if (todayTrades.length > 5) {
    ...
    if (todayPnl < 0) {
      alerts.push({
        id: 'overtrade-today',
        type: 'danger',
        message: `You've taken ${todayTrades.length} trades today with negative PnL. Consider stopping.`,
        timestamp: now,
      });
    } else {
      alerts.push({
        id: 'many-trades-today',
        type: 'warning',
        message: `${todayTrades.length} trades today. Make sure each one follows your plan.`,
        timestamp: now,
      });
    }
  }
  ...
  if (lossStreak >= 3) {
    alerts.push({
      id: 'loss-streak',
      type: 'danger',
      message: `${lossStreak} consecutive losses. Take a break and reassess your strategy.`,
      timestamp: now,
    });
  }
  ...
  if (sorted.length >= 3) {
    ...
    if (
      ...
      risks[1] > 0 &&
      risks[0] > risks[1] * 1.3 &&
      normalizeStatus(sorted[1].trade_status) === 'loss'
    ) {
      alerts.push({
        id: 'risk-increase',
        type: 'warning',
        message: 'Your risk is increasing after losses. Stick to your risk rules.',
        timestamp: now,
      });
    }
  }
  ...
  const weekTrades = trades.filter(
    (t) => new Date(t.trade_date ?? t.created_at ?? '').getTime() >= weekAgo
  );
  const weekMistakes = weekTrades.filter(
    (t) => t.mistake_category && t.mistake_category !== 'No mistake'
  );
  if (weekMistakes.length >= 4) {
    alerts.push({
      id: 'rule-breaks',
      type: 'warning',
      message: `You broke your trading rules ${weekMistakes.length} times this week.`,
      timestamp: now,
    });
  }
...
function determineTradingPersonality(trades: DbTrade[]): string {
  ...
  const winRate =
    totalDecided > 0
      ? (trades.filter((t) => normalizeStatus(t.trade_status) === 'win').length / totalDecided) *
        100
      : 0;
  const dayMap = new Map<string, number>();
  trades.forEach((t) => {
  ...
  const noMistakeRate =
    trades.filter((t) => !t.mistake_category || t.mistake_category === 'No mistake').length /
    trades.length;
  const emotionalTrades = trades.filter((t) =>
    ['revenge', 'greedy', 'fearful', 'anxious'].includes((t.emotion_before ?? '').toLowerCase())
  ).length;
...
function generateWeeklyReport(trades: DbTrade[]): WeeklyReport | null {
  ...
  const totalDecided = weekTrades.filter((t) =>
    ['win', 'loss'].includes(normalizeStatus(t.trade_status))
  ).length;
  ...
  const mistakes = weekTrades.filter(
    (t) => t.mistake_category && t.mistake_category !== 'No mistake'
  );
  ...
  const calmCount = emotionalTrades.filter((t) =>
    ['calm', 'confident', 'neutral'].includes((t.emotion_before ?? '').toLowerCase())
  ).length;
  const emotionalTrend =
    emotionalTrades.length > 0
      ? calmCount / emotionalTrades.length > 0.6
        ? 'Stable'
        : 'Volatile'
      : 'No data';
  ...
  return {
    ...
    mainImprovement:
      pnl > 0
        ? 'Profitable week – keep it consistent'
        : 'Focus on reducing losses and being more selective',
    emotionalTrend,
    recommendations: [
      pnl < 0
        ? 'Review your losing trades and identify common patterns'
        : 'Document what worked well this week',
      mistakes.length > 3
        ? 'Create a pre-trade checklist to avoid recurring mistakes'
...
function identifyStrengths(trades: DbTrade[]): string[] {
  ...
  if (avgWin > avgLoss * 1.5)
    strengths.push(`Winners are ${(avgWin / avgLoss).toFixed(1)}x larger than losers`);

  const noMistake = trades.filter(
    (t) => !t.mistake_category || t.mistake_category === 'No mistake'
  ).length;
  if (noMistake / trades.length > 0.7)
    strengths.push('High discipline – following trading rules consistently');

  const calmTrades = trades.filter((t) => (t.emotion_before ?? '').toLowerCase() === 'calm');
  if (calmTrades.length > trades.length * 0.5)
    strengths.push('Emotionally controlled – trading with calm mindset');

  if (strengths.length === 0)
    strengths.push('Building your trading edge – keep journaling consistently');
...
function identifyWeaknesses(trades: DbTrade[]): string[] {
  ...
  const totalDecided = wins.length + losses.length;
  const winRate = totalDecided > 0 ? (wins.length / totalDecided) * 100 : 0;
  if (winRate < 45 && totalDecided > 5)
    weaknesses.push(`Low win rate of ${winRate.toFixed(1)}% – review entry criteria`);
  ...
  const avgLoss =
    losses.length > 0
      ? losses.reduce((a, t) => a + Math.abs(getTradePnL(t)), 0) / losses.length
      : 0;
  if (avgLoss > avgWin * 1.5 && avgWin > 0)
    weaknesses.push(`Losses ${(avgLoss / avgWin).toFixed(1)}x bigger than wins – tighten stops`);

  const revengeTrades = trades.filter((t) => (t.emotion_before ?? '').toLowerCase() === 'revenge');
  if (revengeTrades.length > 0)
    weaknesses.push(
      `${revengeTrades.length} revenge trades detected – emotional discipline needed`
    );

  const mistakes = trades.filter((t) => t.mistake_category && t.mistake_category !== 'No mistake');
  if (mistakes.length / trades.length > 0.3)
    weaknesses.push(
      `Trading rules broken in ${Math.round((mistakes.length / trades.length) * 100)}% of trades`
    );

  if (weaknesses.length === 0)
    weaknesses.push('No major weaknesses detected – maintain consistency');
  return weaknesses.slice(0, 5);
}

function generateRecommendations(trades: DbTrade[]): string[] {
  if (trades.length === 0)
    return ['Start by logging at least 10 trades to get personalized recommendations'];
  const recs: string[] = [];

  const scores = computeScores(trades);
  if (scores.disciplineScore < 60)
    recs.push('Create a pre-trade checklist and commit to following it every trade');
  if (scores.riskManagementScore < 60)
    recs.push('Standardize your risk per trade – keep it consistent at 1-2% of capital');
  if (scores.emotionalControlScore < 60)
    recs.push('Practice mindfulness before trading sessions and set a max daily loss limit');
  if (scores.consistencyScore < 60)
    recs.push('Set a fixed trading schedule and stick to your preferred sessions');
  ...
  if (bestDay && worstDay && bestDay.day !== worstDay.day && worstDay.pnl < 0) {
    recs.push(
      `Consider avoiding ${worstDay.day}s – your worst performing day. Focus on ${bestDay.day}s instead.`
    );
  }
...
export function useAiInsights(trades: DbTrade[]): {
  ...
  const insights = useMemo<AiInsightsResult | null>(() => {
    ...
    try {
      ...
      const overtradingDetected = Array.from(dayTradeCount.values()).some(
        (d) => d.count > 5 && d.pnl < 0
      );

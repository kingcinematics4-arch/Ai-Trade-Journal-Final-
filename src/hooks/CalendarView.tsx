      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[11px] font-black text-muted-foreground/30 uppercase tracking-[0.2em] mt-1">
              Trading Schedule & Sessions
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="flex bg-blue-500/5 p-1.5 rounded-2xl border border-blue-500/10">
            <button
              ...
              className="p-2.5 hover:bg-blue-500/10 rounded-xl transition-colors text-zinc-400 hover:text-white"
            >
              <ChevronLeft size={18} />
            </button>
            <button
              onClick={handleToday}
              className="px-5 text-xs font-black hover:bg-blue-500/10 rounded-xl transition-colors text-zinc-400 hover:text-white"
            >
              Today
            </button>
            <button
          ...
          <button
            ...
            className="btn-primary h-12 px-6 text-xs font-black rounded-2xl shadow-2xl shadow-blue-500/20"
          >
      ...
      <div className="card-premium overflow-hidden border-white/[0.05] bg-zinc-900/40 backdrop-blur-3xl">
        <div className="grid grid-cols-7 border-b border-blue-500/10 bg-blue-500/5">
          {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((d) => (
            <div
              key={d}
              className="py-4 text-center text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/40"
            >
              {d}
            </div>
          ))}
            ...
            return (
              ...
              >
                ...
                <div className="mt-2 space-y-1 overflow-hidden h-[calc(100%-32px)]">
                  ...
                  {dayEvents.length > 3 && (
                    <div className="text-[9px] text-muted-foreground/50 font-black px-1 uppercase tracking-tighter">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="flex items-center justify-between mb-6">
            ...
            <button
              onClick={handleAddEvent}
              className="text-xs font-black text-blue-500 hover:text-blue-400 transition-colors uppercase tracking-widest"
            >
              Add Event
            </button>
          </div>
            ...
            {getEventsForDay(selectedDate).length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-center border border-dashed border-white/[0.05] rounded-[24px]">
                ...
                <p className="text-sm text-zinc-500 font-medium">
                  No trading events scheduled for this day.
                </p>
              </div>
            ) : (
              getEventsForDay(selectedDate).map((event) => (
                ...
                >
                  <div className="flex items-center gap-5">
                    ...
                    />
                    <div>
                      <p className="font-bold text-[15px] text-white group-hover:text-blue-500 transition-colors">
                        {event.title}
                      </p>
                      <div className="flex items-center gap-3 mt-1 text-xs text-zinc-500">
                        ...
                        {event.description && (
                          <span className="truncate max-w-[200px] italic">{event.description}</span>
                        )}
                      </div>
                    </div>
                  </div>

  ...
  function walk(obj: any, prefix = ''): void {
    ...
    Object.keys(obj).forEach(key => {
      ...
      if (value === null || value === undefined) {
        ...
        } else if (newKey.match(/pnl_amount|risk_amount|entry_price|exit_price|fees|commission|gross_pnl|net_pnl/i)) {
...
export function getProfessionalColumnOrder(keys: string[], isTrades: boolean = true): string[] {
  const priorityKeys = isTrades ? [
    'trade_date', 'trade_time', 'asset_name', 'pnl_amount', 'pnl_percent', 'risk_amount', 
    'strategy_used', 'trade_direction', 'trade_status', 'id', 'symbol', 'market', 
    'exchange', 'asset_type', 'side', 'entry_price', 'exit_price', 'stop_loss', 
    'take_profit', 'position_size', 'quantity', 'leverage', 'reward_amount', 'rr_ratio',
    ...
    'lessons_learned', 'goal.title', 'goal.status', 'task.title', 'task.status', 

...
export function calculateGoalProgress(
  ...
  const relevantTrades = trades.filter(t => 
    t.goalId === goal.id || t.goal_id === goal.id
  );

  console.log(`[Goal Engine Debug] Goal: "${goal.title}", Relevant Trades found: ${relevantTrades.length}`);
  ...
  switch (goal.type) {
    ...
    case 'win_rate':
      ...
      const totalDecided = relevantTrades.filter(t => ['win', 'loss'].includes(normalizeStatus(t.trade_status))).length;
    ...
    case 'rr_ratio':
      const rrs = relevantTrades
        .map(t => parseSafeNumber(t.rr_ratio))
        .filter(rr => rr > 0);
    ...
    case 'discipline':
      const noMistakeCount = relevantTrades.filter(t => (t.mistake_category ?? '').toLowerCase() === 'no mistake' || !t.mistake_category).length;
      const discScore = relevantTrades.length > 0 ? (noMistakeCount / relevantTrades.length) * 100 : 0;

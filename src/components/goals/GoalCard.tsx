...
export default function GoalCard({ goal, onEdit, onDelete }: GoalCardProps) {
  const getIcon = () => {
    switch (goal.category) {
      ...
      case 'risk': return <ShieldCheck className="w-5 h-5 text-blue-500" />;
      case 'psychology': return <Zap className="w-5 h-5 text-purple-500" />;
  ...
  const getStatusColor = () => {
    switch (goal.status) {
      ...
      case 'failed': return 'bg-loss/20 text-loss border-loss/30';
      ...
      <div className="flex justify-between items-start mb-4 relative z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className={`text-xs px-2 py-0.5 rounded-full border ${getStatusColor()} capitalize font-medium`}>
              {goal.status}
            </span>
          </div>
        </div>
      ...
      <div className="space-y-2 relative z-10">
        <div className="flex justify-between text-sm">
          ...
          <span className="font-semibold text-foreground">
            ...
            {(goal.currentValue || 0).toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 2 })} / {(goal.targetValue || 0).toLocaleString()}
          </span>
        </div>

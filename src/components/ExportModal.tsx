    ...
    try {
      ...
      const success = await exportData(
        data,
        {
          ...
          selectedFields:
            category === 'trades' || selectedOptionalFields.length > 0
      ...
      toast.success(`Professional ${format.toUpperCase()} exported successfully!`, {
        id: loadingToast,
      });
      onExportSuccess?.();
      onClose();
      ...
      >
        <div className="px-6 py-5 border-b border-border flex items-center justify-between bg-muted/30">
          <div className="flex items-center gap-3">
            ...
            <h3 className="font-black uppercase tracking-[0.2em] text-sm text-foreground">
              {t('export.title')}
            </h3>
          </div>
          <button
            onClick={onClose}
            className="p-2 hover:bg-muted rounded-full transition-all duration-200 text-muted-foreground hover:scale-110 active:scale-95"
          >
            <X size={20} />
          </button>
        </div>
            ...
            <div className="flex flex-col h-full min-h-[500px]">
              <div className="flex flex-col h-full">
                <div className="flex items-center justify-between mb-4">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">
                    {t('export.exportFields')}
                  </label>
                  <div className="flex gap-2 relative">
                    <button
                      onClick={quickActions.tradingEssentials}
                    ...
                    <button
                      onClick={quickActions.clear}
                      className="text-[9px] font-bold text-muted-foreground hover:text-white transition-all"
                    >
                      {t('export.clear')}
                    </button>
                    ...
                    {deletedFilters.length > 0 && (
                      ...
                      >
                        {t('export.editBin')} ({deletedFilters.length})
                      </button>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-muted/5 border border-border rounded-2xl p-4 space-y-4 max-h-[400px] overflow-y-auto custom-scrollbar">
                  {category === 'trades' && (
                    <div className="space-y-2 mb-4">
                      <p className="text-[9px] font-black text-muted-foreground/40 uppercase tracking-widest flex items-center gap-1.5">
                        <ListChecks size={10} /> {t('export.mandatoryFields')}
                      </p>
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                        {TRADE_PERMANENT_KEYS.map((key) => (
                          <div
                            key={key}
                            className="flex items-center gap-2 p-2 rounded-lg bg-white/[0.02] border border-white/[0.03] opacity-60"
                          >
                  ...
                  <div className="space-y-2">
                    <p className="text-[9px] font-black text-muted-foreground/40 uppercase tracking-widest flex items-center gap-1.5">
                      <Filter size={10} /> {t('export.optionalDataPoints')}
                    </p>
                        ...
                        {availableOptionalFields.map((field) => (
                          ...
                          >
                            ...
                            >
                              ...
                              <span
                                className={`text-[10px] font-bold truncate ${selectedOptionalFields.includes(field) ? 'text-primary' : 'text-muted-foreground'}`}
                              >
                                {formatLabel(field)}
                              </span>
                            </button>
        ...
        <div className="p-6 bg-muted/30 border-t border-border flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-white/30 uppercase tracking-widest">
                {t('export.reportDepth')}
              </span>
              <span className="text-xs font-bold text-white">
                {format === 'pdf' ? pdfReportType.toUpperCase() : 'N/A'}
              </span>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            <button
              onClick={onClose}
              className="px-6 py-3 text-xs font-bold text-muted-foreground hover:text-white transition-colors"
            >
              {t('export.cancel')}
            </button>
            <button

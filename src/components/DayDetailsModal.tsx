...
export default function DayDetailsModal({
  ...
  const { netPnL, winRate } = useMemo(() => {
    ...
    const wr =
      trades.length > 0
        ? (trades.filter((t) => normalizeStatus(t.trade_status || t.result) === 'win').length /
            trades.length) *
          100
        : 0;
          ...
          <div className="flex-1 overflow-y-auto p-8 pt-6 space-y-10 scrollbar-thin scrollbar-thumb-white/[0.05] scrollbar-track-transparent custom-scrollbar">
            ...
            <motion.div variants={itemVariants} className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="p-5 rounded-[24px] bg-white/[0.02] border border-white/[0.04] flex flex-col gap-3 group hover:bg-white/[0.04] transition-all relative overflow-hidden">
                ...
                <div
                  className={cn(
                    'absolute bottom-0 left-0 h-[2px] w-full bg-gradient-to-r from-transparent via-blue-500/20 to-transparent scale-x-0 group-hover:scale-x-100 transition-transform duration-500'
                  )}
                />
              </div>
              ...
              <div className="p-5 rounded-[24px] bg-white/[0.02] border border-white/[0.04] flex flex-col gap-3 group hover:bg-white/[0.04] transition-all">
                ...
                <div className="text-2xl font-black text-white tabular-nums tracking-tighter">
                  {trades.length}{' '}
                  <span className="text-xs text-muted-foreground/40 font-bold ml-1 uppercase">
                    Positions
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {goals.length > 0 ? (
                  goals.map((goal) => (
                    <div
                      key={goal.id}
                      className="p-6 rounded-[28px] bg-white/[0.02] border border-white/[0.06] hover:border-emerald-500/30 hover:bg-white/[0.04] transition-all group relative overflow-hidden"
                    >
                      ...
                      <div className="flex items-center justify-between mt-5">
                        ...
                        <Target
                          size={14}
                          className="text-white/10 group-hover:text-emerald-500/30 transition-colors"
                        />
                      </div>
                    </div>
                  ))
                  ...
                  <div className="col-span-full py-10 flex flex-col items-center justify-center rounded-[28px] border border-dashed border-white/[0.06] bg-white/[0.01]">
                    ...
                    <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest opacity-40">
                      Zero active targets for this cycle
                    </p>
                  </div>
                )}
              </div>
                ...
                {trades.length > 0 ? (
                  trades.map((trade) => {
                    ...
                    return (
                      <div
                        key={trade.id}
                        className="group relative p-6 rounded-[28px] bg-white/[0.02] border border-white/[0.06] hover:bg-white/[0.04] transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-6 overflow-hidden"
                      >
                        <div className="flex items-center gap-5">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-[0.2em] mt-1.5 flex items-center gap-2">
                              ...
                              <span className="w-1 h-1 rounded-full bg-white/10" /> STRTGY:{' '}
                              {trade.strategy_used || 'Standard'}
                            </p>
                          </div>
                        </div>
                  ...
                  <div className="py-12 flex flex-col items-center justify-center rounded-[28px] border border-dashed border-white/[0.06] bg-white/[0.01]">
                    ...
                    <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest opacity-40">
                      No market execution logs found
                    </p>
                  </div>
                )}
              </div>
            ...
            <motion.section variants={itemVariants} className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  ...
                  <div
                    key={stat.label}
                    className="p-4 rounded-2xl bg-white/[0.01] border border-white/[0.03] flex flex-col items-center justify-center gap-1 group hover:bg-white/[0.03] transition-all"
                  >
                    <span className="text-[8px] font-black text-muted-foreground/40 uppercase tracking-widest">
                      {stat.label}
                    </span>
                    <span
                      className={cn('text-lg font-black tracking-tighter', {
                ...
                {isAddingTask && (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-3">
                            ...
                            <button
                              ...
                              className={cn(
                                ...
                                newTaskData.priority === p
                                  ? PRIORITY_COLORS[p]
                                  : 'border-transparent text-muted-foreground/40 hover:text-white'
                              )}
                            >
                              {p}
                            </button>
                        ...
                        <div className="flex items-center gap-2 bg-white/[0.05] px-2 py-1 rounded-lg text-muted-foreground">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setNewTaskData({ ...newTaskData, startTime: e.target.value })
                            }
                          />
                        </div>
                      </div>
                    ...
                    <textarea
                      ...
                      className="bg-transparent text-xs text-muted-foreground/60 placeholder:text-muted-foreground/20 outline-none w-full resize-none h-16"
                    ...
                    <div className="flex items-center justify-end gap-3 pt-2">
                      <button
                        ...
                        className="text-[10px] font-black text-muted-foreground uppercase tracking-widest hover:text-white"
                      >
                        Cancel
                      </button>
                      <button
                        onClick={handleAddTask}
                        className="px-4 py-2 rounded-xl bg-blue-500 text-white text-[10px] font-black uppercase tracking-widest shadow-lg shadow-blue-500/20 active:scale-95 transition-all"
                      >
                        Save Task
                      </button>
                    </div>
                ...
                {filteredTasks.length > 0 ? (
                  filteredTasks.map((event) => (
                    <motion.div
                      ...
                      key={event.id}
                      className={cn(
                        'p-5 rounded-[24px] bg-white/[0.02] border border-white/[0.05] flex items-center justify-between hover:bg-white/[0.04] transition-all group',
                    ...
                    >
                      <div className="flex items-center gap-4 flex-1">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            onUpdateTask(event.id, { ...event, completed: !event.completed });
                          }}
                          className={cn(
                        ...
                        <div className="flex-1">
                          ...
                          {event.description && (
                            <p
                              className={cn(
                                'text-[11px] text-muted-foreground/40 mt-1 line-clamp-1',
              ...
              <motion.section variants={itemVariants} className="space-y-5">
                ...
                <div className="p-6 rounded-[28px] bg-white/[0.02] border border-white/[0.05] relative overflow-hidden group">
                  ...
                  <div className="text-sm text-zinc-400 leading-relaxed font-medium italic">
                    "Institutional performance summary for {format(date, 'MMM do')}: Markets showed
                    significant volatility. Key lessons derived from execution logs should be
                    applied to the next trading cycle."
                  </div>
                  <div className="mt-6 pt-6 border-t border-white/[0.03]">
                    <div className="flex items-center gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                      <span className="text-[10px] font-black text-white/30 uppercase tracking-widest">
                        AI Performance Synthesizer Active
                      </span>
                    </div>
                  </div>
                </div>

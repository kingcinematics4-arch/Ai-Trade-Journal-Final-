...
export default function CalendarView() {
  ...
  const getTradesForDay = (day: Date) => {
    ...
    return trades.filter((t: Trade) => (t.trade_date || t.date) === format(day, 'yyyy-MM-dd'));
  };
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className="text-[9px] text-[#64748b] font-bold uppercase tracking-[0.15em]">
                Terminal System
              </span>
              <span className="w-1 h-1 rounded-full bg-white/[0.05]" />
              <span className="text-[9px] text-blue-500/50 font-bold uppercase tracking-[0.15em]">
                Performance Active
              </span>
            </div>
          </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex bg-[#0b0f1a] border border-white/[0.05] p-0.5 rounded-sm">
            <button
              ...
              className="p-1.5 hover:bg-white/[0.03] text-[#64748b] transition-colors"
            ...
            <button
              onClick={handleToday}
              className="px-3 py-1 text-[10px] font-black uppercase tracking-tighter hover:bg-white/[0.03] text-[#94a3b8] transition-colors border-x border-white/[0.05]"
            >
              Today
            </button>
            <button
            ...
            <button
              ...
              className={cn(
                ...
                isFilterOpen
                  ...
                  : 'bg-[#0b0f1a] border-white/[0.05] text-[#94a3b8] hover:text-white'
              )}
            >
              ...
              {isFilterOpen && (
                ...
                >
                  <div className="flex items-center justify-between px-2.5 py-2 border-b border-white/[0.05] mb-1">
                    <span className="text-[9px] font-black uppercase text-[#64748b] tracking-wider">
                      Data Segments
                    </span>
                    <button
                      onClick={clearFilters}
                      className="text-[9px] font-bold text-blue-500 hover:text-blue-400 transition-colors"
                    >
                      Reset
                    </button>
                  </div>
      ...
      <div className="w-full min-w-0 border-0 bg-transparent">
        <div className="grid grid-cols-7 gap-4 bg-transparent mb-4">
          {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day) => (
            <div
              key={day}
              className="py-2 text-center text-[10px] font-bold uppercase tracking-[0.14em] text-[#8da2c0]"
            >
              {day}
            </div>
          ))}
        ...
        <div className="grid grid-cols-7 w-full gap-4 auto-rows-auto">
          {days.map((day) => {
            ...
            const wins = dayTrades.filter(
              (t) => normalizeStatus(t.trade_status || t.result) === 'win'
            ).length;
            const losses = dayTrades.filter(
              (t) => normalizeStatus(t.trade_status || t.result) === 'loss'
            ).length;

            const filterAll = activeFilters.includes('all');
            const showPnL =
              filterAll ||
              activeFilters.includes('dailyPnL') ||
              activeFilters.includes('performance');
            const showTrades =
              filterAll ||
              activeFilters.includes('tradeCount') ||
              activeFilters.includes('performance');
            const showProfitLossCount =
              filterAll ||
              activeFilters.includes('profitTrades') ||
              activeFilters.includes('lossTrades') ||
              activeFilters.includes('performance');
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  'h-auto',
                  dayTrades.length === 0 && dayEvents.length === 0 && dayGoals.length === 0
                    ? 'min-h-[80px] opacity-70 hover:opacity-100'
                    : 'min-h-[110px]',
                  ...
                  dayTrades.length > 0 &&
                    netPnL > 0 &&
                    'border-l-[4px] border-l-emerald-400 shadow-[0_0_40px_rgba(16,185,129,0.10)] before:bg-[radial-gradient(circle_at_top_left,rgba(16,185,129,0.18),transparent_60%)]',
                  dayTrades.length > 0 &&
                    netPnL < 0 &&
                    'border-l-[4px] border-l-red-400 shadow-[0_0_40px_rgba(239,68,68,0.10)] before:bg-[radial-gradient(circle_at_top_left,rgba(239,68,68,0.18),transparent_60%)]'
                )}
              >
                ...
                <div className="mt-1 flex-1 overflow-hidden pointer-events-none">
                  {dayEvents.slice(0, 1).map((e) => (
                    <div
                      key={e.id}
                      className="text-[8px] font-bold text-zinc-600 truncate leading-tight uppercase tracking-tighter group-hover:text-zinc-400 transition-colors"
                    >
                      {e.title}
                    </div>
                  ))}
                ...
                <div className="mt-auto flex flex-col items-end leading-none space-y-0.5">
                  {(showProfitLossCount || showTrades) && dayTrades.length > 0 && (
                    <div className="flex items-center gap-1.5 opacity-60 group-hover:opacity-100 transition-opacity">
                      {showTrades && (
                        <span className="text-[9px] font-semibold text-[#9db0cf] uppercase tracking-tight">
                          {dayTrades.length}T
                        </span>
                      )}
        ...
        {selectedDate && (
          ...
          >
            <div className="flex items-center justify-between border-b border-white/[0.04] pb-3">
              <div className="flex items-center gap-3">
                <span className="text-[11px] font-black text-blue-500/80 uppercase tracking-widest leading-none">
                  Activity Stream // {format(selectedDate, 'dd.MM.yy')}
                </span>
                {isToday(selectedDate) && (
                  <span className="bg-blue-600/10 text-blue-500 text-[8px] font-black px-1.5 py-0.5 rounded-sm uppercase tracking-wider border border-blue-500/20">
                    Today
                  </span>
                )}
              </div>
            ...
            <div className="space-y-3">
              ...
              <CalendarTable
                ...
                data={getEventsForDay(selectedDate).sort((a, b) =>
                ...
                renderRow={(event: CalendarEvent) => (
                  ...
                  >
                      ...
                      >
                        <CheckCircle2
                          ...
                          className={cn(
                            ...
                            event.completed ? 'scale-100' : 'scale-0'
                          )}
                        />
                      </button>
                    </td>
                    <td
                      className={cn(
                        'px-4 py-3 font-bold text-zinc-300 transition-all',
                        event.completed && 'line-through text-zinc-500'
                      )}
                    >
                      {event.title}
                    </td>
                    <td className="px-4 py-3 text-zinc-600 font-mono text-[11px] whitespace-nowrap tabular-nums">
                      {event.startTime}
                    </td>
                    <td className="px-4 py-3 text-right text-zinc-600 truncate max-w-[200px] text-[11px]">
                      {event.description || '-'}
                    </td>
                  </tr>
                )}
            ...
            <div className="space-y-3">
              ...
              <CalendarTable
                ...
                renderRow={(trade: any) => (
                  <tr
                    key={trade.id}
                    className="hover:bg-zinc-900/50 border-b border-zinc-900/50 last:border-0 transition-colors"
                  >
                    ...
                    <td className="px-4 py-2 text-right">
                      <span
                        className={cn(
                          'font-black font-mono tracking-tight tabular-nums',
                          getTradePnL(trade) >= 0 ? 'text-green-500' : 'text-red-500'
                        )}
                      >
                        {getTradePnL(trade) >= 0 ? '+' : ''}
                        {getTradePnL(trade).toLocaleString(undefined, {
            ...
            <div className="space-y-3">
              ...
              <CalendarTable
                ...
                renderRow={(goal: any) => (
                  <tr
                    key={goal.id}
                    className="hover:bg-zinc-900/50 border-b border-zinc-900/50 last:border-0 transition-colors cursor-pointer"
                    onClick={() => {
                      setEditingGoal(goal);
                      setIsGoalModalOpen(true);
                    }}
                  >
                    ...
                    <td className="px-4 py-2 text-right">
                      <span
                        className={cn(
                          'text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-sm tabular-nums',
                          goal.status === 'completed'
                            ? 'bg-green-950/40 text-green-500 border border-green-500/20'
                            : 'bg-blue-950/40 text-blue-500 border border-blue-500/20'
                        )}
                      >
                        {goal.status}
                      </span>
